...
def add_index_and_test():
    ...
    dataset_maxar = NpyDataset(base_dir+"maxar_irrigation_tiles", base_dir+"maxar_urban_tiles", base_dir+"maxar_snow_cap_mountain_tiles", False, True, 10000, 10050, transform1, transform2)
    test_loader = DataLoader(dataset_maxar, batch_size=batch_size, shuffle=True)
    print(len(dataset_maxar))
    ...
    conn.close()
    print(retrieved_embeddings_np_maxar.shape)
    ...
    for inputs in test_loader:
        ...
        z1_online, z2_online, z1_target, z2_target  = maxar_model(x1, x2)
        print(z1_online.shape)

        query_embeddings_z1 = z1_online.detach().numpy()
        query_embeddings_z2 = z2_online.detach().numpy()
        print(query_embeddings_z1.shape)

        distances1, indices1 = index_maxar.search(query_embeddings_z1, k)
        distances2, indices2 = index_maxar.search(query_embeddings_z2, k)
        i_m = 0
        corr_class1 = []
        ...
        if corr_class1:
            ...
            if planet_class == cur_type:
                ...
                if "irrigation" in cur_type:
                    dir_req_cur = "maxar_irrigation_tiles/"
                
                if "snow" in cur_type:
                    dir_req_cur = "maxar_snow_cap_mountain_tiles/"

                if "urban" in cur_type:
                    dir_req_cur = "maxar_urban_tiles/"
        ...
        if corr_class2:
            maxar_class = max(corr_class2,key=corr_class2.count)
            if maxar_class == cur_type:
                print(cur_type)
                print(maxar_class)
                accuracy+=1
            ...
            preds.append(maxar_class)
            actuals.append(cur_type)  


    #TODO: from the dataset accuracy is calculated as one for planet and one for maxar
    print("accuracy = ",accuracy/(len(dataset_maxar)))
    print("ssim = ",total_ssim/(len(dataset_maxar)))
    print("preds length: ",len(preds))
    print("actuals length: ",len(actuals))
    plot_mertics(actuals, preds, "Two Encoder Based (Maxar Only)")

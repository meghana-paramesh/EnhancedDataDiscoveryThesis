...
def train():
   ...
   dataset_planet = NpyDataset(base_dir+"planet_irrigation_tiles", base_dir+"planet_urban_tiles", base_dir+"planet_snow_cap_mountains_tiles", True, False, 0, 20000, transform1, transform2)

   train_size = int(0.9 * len(dataset_planet))
   validation_size = len(dataset_planet) - train_size
   train_dataset, validation_dataset = torch.utils.data.random_split(dataset_planet, [train_size, validation_size])
   ...
   for epoch in range(num_epochs):
      ...
      torch.save(checkpoint, 'planet_contrastive_loss.pth')
      print(f"Epoch [{epoch}/{num_epochs}] Training Loss: {train_loss:.4f} Validation Loss: {val_loss:.4f}")
   
   plot_training_validation_loss(training_loss, validation_loss, "planet_contrastive_loss", 3)

...
def add_index_and_test():
    ...
    dataset_planet = NpyDataset(base_dir+"planet_irrigation_tiles", base_dir+"planet_urban_tiles", base_dir+"planet_snow_cap_mountains_tiles", False, True, 10000, 10050, transform1, transform2)
    test_loader = DataLoader(dataset_planet, batch_size=batch_size, shuffle=True)
    print(len(dataset_planet))
    ...
    conn.close()
    print(retrieved_embeddings_np_planet.shape)
    ...
    for inputs in test_loader:
        ...
        distances1, indices1 = index_planet.search(query_embeddings_z1, k)
        distances2, indices2 = index_planet.search(query_embeddings_z2, k)
        i_m = 0
        corr_class1 = []
        for idx_m in indices1[0]:
            corresponding_tile_type = complete_result_planet[idx_m][3]
                
            corresponding_tile_name = complete_result_planet[idx_m][2][2:-3]
        ...
        if corr_class1:
            ...
            if planet_class == cur_type:
                ...
                if "irrigation" in cor_type:
                    dir_req_cor = "planet_irrigation_tiles/"
                
                if "snow" in cor_type:
                    dir_req_cor = "planet_snow_cap_mountains_tiles/"

                if "urban" in cor_type:
                    dir_req_cor = "planet_urban_tiles/"
        ...
        for idx_p in indices2[0]:   
            corresponding_tile_name = complete_result_planet[idx_p][2][2:-3]
            cor_type, cor_filename = get_file_and_type(corresponding_tile_name)
            cur_tile_name = str(filename)[2:-3]
            cur_type, cur_filename = get_file_and_type(cur_tile_name)
            print("\n")

...
from ModelWithSelfAttentionEncoder import SimSiam
...
def train_self_attentionsimsiam():
    ...
    training_loss_tracker = open("training_loss_tracker1.txt", "a")
    validation_loss_tracker = open("validation_loss_tracker1.txt", "a")
    ...
    for i in range(0, min_len, batch_files):
       ...
       checkpoint_path = 'model_self_attaention_trail1.pt'
       model = SimSiam(8)
       model.to(device)
       ...
       for epoch in range(num_epochs):
          ...
          for batch in data_loader:
             ...
             proj1, proj2, pred1, pred2 = model(maxar_tile, planet_tile)


             # Compute the loss
             loss = criterion(proj1, proj2) + criterion(pred1, pred2)
          ...
          for val_batch in validation_data_loader:
             ...
             # Forward pass
             val_p1, val_p2 = model(val_maxar_tile, val_planet_tile)

             # Compute the loss
             val_loss = 1 - criterion(p1, p2).mean()
             val_total_loss += val_loss.item()
          val_avg_loss = val_total_loss / len(validation_dataset)

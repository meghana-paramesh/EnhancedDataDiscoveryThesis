...
def tile_extractor():
    ...
    maxar_tiles_base_dir = "/home/CS/mp0157/dataset/corresponding_tiles/maxar_irrigation_tiles/"
    planet_tiles_base_dir = "/home/CS/mp0157/dataset/corresponding_tiles/planet_irrigation_tiles/"


    list_maxar = file_list_maxar_irrigation[0:200]
                ...
                with rasterio.open(planet_file_path) as planet_ds, rasterio.open(maxar_file_path) as maxar_ds:
                    ...
                    for y in range(0, height, tile_size[0]):
                        for x in range(0, width, tile_size[1]):
                            m_tile_data = resized_planet_data[:, y:y + tile_size[0], x:x + tile_size[1]].astype(np.float32)
                            p_tile_data = resized_maxar_data[:, y:y + tile_size[0], x:x + tile_size[1]].astype(np.float32)
                            if m_tile_data.shape[1]==256 and p_tile_data.shape[1]==256 and m_tile_data.shape[2]==256 and p_tile_data.shape[2]==256:
                                if m_tile_data.size>0 and p_tile_data.size>0 and not contains_mostly_zero(m_tile_data) and not contains_mostly_zero(p_tile_data):
